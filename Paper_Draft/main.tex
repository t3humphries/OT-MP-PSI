...
We present a variant with communication complexity $O(nmk)$ and round complexity $O(\log{k} + 1)$.
However, the constants in reconstruction prevent scaling to practically relevant problem sizes.
Our improved scheme has communication complexity $O(nmkt)$ and round complexity $O(\log{k} + 1)$.
