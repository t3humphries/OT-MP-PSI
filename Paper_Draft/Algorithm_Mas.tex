...
\begin{figure}[h!]
   ...
   \begin{tikzpicture}
    ...
    % Message from B to A, with computations by both sides
      ...
      \postlevel
      \postlevel
      \mess{A}{\shortstack[l]{$\{g^{r_j \alpha} H(l)^{c_j \alpha}\}_{j = 0, \dots, t-1}$\\$\{g^{r'_j \alpha} H(l)^{c'_j \alpha}\}_{j = 0, \dots, t-1}$ }}{B}
      ...
      $\{g^{r_j} H(l)^{c_j}\}_{j = 0, \dots, t-1}$\\
      $\{g^{r'_j} H(l)^{c'_j}\}_{j = 0, \dots, t-1}$
      }}; 
      ...
      \postlevel
      \postlevel
      \mess{B}{\shortstack[c]{$\{Enc_{pk}[g^{r_j} (id_i)^j  H(l)^{c_j}]\}_{j = 0, \dots, t-1}$\\ $\{Enc_{pk}[g^{r'_j} (id_i)^j  H(l)^{c'_j}]\}_{j = 0, \dots, t-1}$}}{A}
      ...
      \vspace{0.8em}\\
      $\{Enc_{pk}[(id_i)^j H(l)^{c_j}]\}_{j = 0, \dots, t-1}$ \vspace{0.5em}\\
      $\{Enc_{pk}[(id_i)^j H(l)^{c'_j}]\}_{j = 0, \dots, t-1}$}};
      ...
      \node[anchor=east] at (mess from) {\shortstack[r]{
       $P(x) = \sum\limits_{j = 0, \dots, t-1} Enc[x^j H(l)^{c_j}] $\\
       \vspace{1em}\\
       $P'(x) = \sum\limits_{j = 0, \dots, t-1} Enc[x^j H(l)^{c'_j}] $}};
