...
\begin{figure}[h!]
   ...
   \begin{tikzpicture}
    ...
    \node[anchor=east] at (mess to) {\shortstack[l]{$H(l)^{\alpha K_1}, \{H(l)^{c_i \alpha}\}_{i = 1, \dots, t-1}$ \\ $H(l)^{\alpha K_1}, \{H(l)^{c'_i \alpha}\}_{i = 1, \dots, t-1}$}};
      
      \postlevel
      \postlevel
      \mess{A}{\shortstack[c]{$P(id)$\\$P'(id)$}}{B}
      \node[anchor=east] at (mess from) {\shortstack[r]{
       $P(x) = H(l)^{\alpha \sum_{}c_i x^i} H(l)^{\alpha K_1}$
       \vspace{.4em}
       \\$P'(x) = H(l)^{\alpha \sum_{}c'_i x^i} H(l)^{\alpha K_1}$}};
            ...
            \ForEach {Participant $id_i$;  $i = 1, \dots, m$}
	 	    ...
	 	    \ForEach {Bin $b_z$;  $z = 1, \dots, b$}
	 	        \ForEach {t-subset of quadruples in the bin $b_z$; $\{(id_{i_1}, Share_{i_1}(l), MAC_{i_1}(l), b_z), \dots, (id_{i_t}, Share_{i_t}(l), MAC_{i_t}(l), b_z)\}$, where $id_{i_i}$s are distinct,}
	 	            ...
	 	            \If {$Secret_{MAC} == Secret_{share}$}{ Reveal that $(id_{i_1}, \dots , id{i_t})$ can reconstruct the $Secret_{share}$ which is $H(l)^{K_1}$}
	 	            \EndIf 
	 	        \EndFor
	 	    \EndFor
	 	\end{algorithmic}
...
\begin{figure}[h!]
   ...
   \begin{tikzpicture}
    ...
    % Message from B to A, with computations by both sides
      ...
      \postlevel
      \postlevel
      \mess{A}{\shortstack[l]{$g^{r_1 \alpha}  H(l)^{K_1 \alpha}$, $\{g^{r_2 \alpha} H(l)^{c_j \alpha}\}_{j = 1, \dots, t-1}$\\$g^{r'_1 \alpha}  H(l)^{K_1 \alpha}$, $\{g^{r'_2 \alpha} H(l)^{c'_j \alpha}\}_{j = 1, \dots, t-1}$ }}{B}
      \node[anchor=east] at (mess from) {\shortstack[c]{
      $r_1, r_2 \gets \mathbb{F}_p$, Global $c_j$s\\
      $r'_1, r'_2 \gets \mathbb{F}_p$, Global $c'_j$s
      }};
      \node[anchor=west] at (mess to) {\shortstack[l]{
      $g^{r_1} H(l)^{K_1}$, $\{g^{r_2} H(l)^{c_j}\}_{j = 1, \dots, t-1}$\\
      $g^{r'_1} H(l)^{K_1}$, $\{g^{r'_2} H(l)^{c'_j}\}_{j = 1, \dots, t-1}$
      }}; 
      ...
      \postlevel
      \postlevel
      \mess{B}{\shortstack[c]{$Enc_{pk}[g^{r_1} H(l)^{K_1}]$,\\ $\{Enc_{pk}[g^{r_2} (id_i)^j  H(l)^{c_j}]\}_{j = 1, \dots, t-1}$\\$Enc_{pk}[g^{r'_1} H(l)^{K_1}]$, \\$\{Enc_{pk}[g^{r'_2} (id_i)^j  H(l)^{c'_j}]\}_{j = 1, \dots, t-1}$}}{A}
      \node[anchor=west] at (mess from) {\shortstack[l]{
            Enc: homomorphic \\encryption scheme}};
      \node[anchor=east] at (mess to) {\shortstack[l]{($\times Enc_{pk}[g^{-r_1}]$, $\times Enc_{pk}[g^{-r_2}]$) : \vspace{0.8em}\\
      \hspace{0.03em} $Enc_{pk}[H(l)^{K_1}]$, \\ $\{Enc_{pk}[H(l)^{c_j}]\}_{j = 1, \dots, t-1}$ \vspace{0.5em}\\
      \hspace{0.03em} $Enc_{pk}[H(l)^{K_1}]$, \\$\{Enc_{pk}[H(l)^{c'_j}]\}_{j = 1, \dots, t-1}$}};
      ...
      \node[anchor=east] at (mess from) {\shortstack[r]{
       $P(x) = \sum\limits_{j = 1, \dots, t-1} Enc[x^j H(l)^{c_j}] $\\
       $ + Enc[H(l)^{K_1}]$
       \vspace{1em}\\
       $P'(x) = \sum\limits_{j = 1, \dots, t-1} Enc[x^j H(l)^{c'_j}] $\\
       $+ Enc[H(l)^{K_1}]$}};
